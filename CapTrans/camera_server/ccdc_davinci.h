...
/* structure for ALaw */
struct a_law {
	int b_alaw_enable;	/* Enable/disable A-Law */
...
struct fault_pixel {
	int fpc_enable;	/*Enable or Disable fault pixel correction */
...
typedef struct ccdc_config_params_raw {
	...
	int image_invert_enable;	/* enable to store the image in inverse
...
static inline int enable_vd_port(int enable)
{
        unsigned int val = regr(FMTCFG);
        if (enable) {
                val |= ENABLE_VIDEO_PORT;	/* enable video port */
        } else {
                val &= ~ENABLE_VIDEO_PORT;  /* disable video port */
        }
        regw(val, FMTCFG);
